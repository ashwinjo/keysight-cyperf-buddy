...
// Returns a flat list of AI CVE records. Fetches up to 1000 records per API limit.
// For proper pagination with >1000 records, implement page-based loading in the component.
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get<{ results: AiCVEResponse[]; total: number }>(
        `${API_BASE}/ai-cves`,
        {
          params: { limit: 1000 }, // Backend max limit; fetch all records for client-side filtering
        }
      );
